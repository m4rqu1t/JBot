...
* **Canais de Voz Dinâmicos:** Escuta eventos de voz para criar e limpar canais temporários automaticamente (evitando poluição visual no servidor).
...
```bash
docker run -d --name janjabot_container -e DISCORD_TOKEN="SEU_TOKEN_AQUI" janjabot

```
### 3. Pare o bot
Se precisar desligar o bot, utilize:
```bash
docker stop janjabot_container
